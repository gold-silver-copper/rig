...
impl Route {
    fn builder(self, client: OpenAiCassette) -> AgentBuilder {
        match self {
            Route::Chat => rig::agent(client.chat.completion(MODEL)),
            Route::Responses => rig::agent(client.openai.completion(MODEL)),
        }
    }
...
async fn chat_retried_turn_records_retried_attempt_raw() {
    ...
    with_openai_cassette(
        ...
        |client| async move {
            let response = rig::agent(client.chat.completion(MODEL))
                .preamble(
