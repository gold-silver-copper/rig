...
async fn shared_tool_server_handle_updates_all_agents() {
    ...
    with_gemini_cassette(
        ...
        |client| async move {
            let handle = ToolServer::new().tool(add).run();
            let first_agent = rig::agent(client.completion(gemini::completion::GEMINI_2_5_FLASH))
                .preamble(FORCE_TOOLS_PREAMBLE)
                .temperature(0.0)
                .tool_server_handle(handle.clone())
                .default_max_turns(3)
                .build();
            let second_agent = rig::agent(client.completion(gemini::completion::GEMINI_2_5_FLASH))
                .preamble(FORCE_TOOLS_PREAMBLE)
                .temperature(0.0)
                .tool_server_handle(handle.clone())
                .default_max_turns(3)
