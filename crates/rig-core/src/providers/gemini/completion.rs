...
mod tests {
    ...
    #[test]
    fn test_default_api_function_call_is_mapped_as_tool_call() {
        let response: GenerateContentResponse = serde_json::from_value(json!({
            "responseId": "resp_default_api",
            "candidates": [{
                "content": {
                    "parts": [{
                        "functionCall": {
        ...
        .expect("Gemini fixture should deserialize");

        let converted: crate::completion::CompletionResponse<GenerateContentResponse> =
            response.try_into().expect("convert response");

        match converted.choice.first() {
            message::AssistantContent::ToolCall(tool_call) => {
                assert_eq!(tool_call.id, "default_api");
                assert_eq!(tool_call.function.name, "default_api");
                assert_eq!(
                    tool_call.function.arguments["code"],
                    "async function findBundle() { return Ryzome.listBundle(\"6a07410250db81016def9dd1\", 1); } findBundle();"
                );
            }
            other => panic!("expected default_api to be mapped as a tool call, got {other:?}"),
        }
    }

    #[test]
    fn test_tool_code_default_api_text_is_not_mapped_as_tool_call() {
        let response: GenerateContentResponse = serde_json::from_value(json!({
            "responseId": "resp_tool_code_text",
            "candidates": [{
                "content": {
                    "parts": [{
                        "text": "```tool_code\nprint(default_api.JavaScript(code=\"findBundle();\"))\n```"
                    }],
                    "role": "model"
                },
                "finishReason": "STOP",
        ...
        match converted.choice.first() {
            message::AssistantContent::Text(text) => {
                assert!(text.text.contains("default_api.JavaScript"));
            }
            other => panic!("tool_code text should remain text, got {other:?}"),
        }
    }
