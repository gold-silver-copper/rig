...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // transport, so this pairs the wire with rig-reqwest's shared one.
    // OpenAI's `completion` wire is the Responses API; `.chat(model)` names
    // the Chat Completions wire instead.
...
catalog is not exhaustive: other models can use `ModelAdapter` directly.
