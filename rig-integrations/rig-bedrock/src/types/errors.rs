...
impl From<AwsSdkInvokeModelError> for ImageGenerationError {
    fn from(value: AwsSdkInvokeModelError) -> Self {
        ImageGenerationError::transport(value.into_service_error())
    }
}

impl From<AwsSdkInvokeModelError> for EmbeddingError {
    fn from(value: AwsSdkInvokeModelError) -> Self {
        EmbeddingError::transport(value.into_service_error())
    }
}
...
impl From<AwsSdkConverseError> for CompletionError {
    fn from(value: AwsSdkConverseError) -> Self {
        ...
        CompletionError::transport(error)
    }
}
...
impl From<AwsSdkConverseStreamError> for CompletionError {
    fn from(value: AwsSdkConverseStreamError) -> Self {
        let error: String = match value.0.into_service_error() {
            ConverseStreamError::ModelTimeoutException(e) => e.message.unwrap_or(
                "The request took too long to process. Processing time exceeded the model timeout length.".into(),
            ),
            ConverseStreamError::AccessDeniedException(e) => e.message.unwrap_or(
                "The request is denied because you do not have sufficient permissions to perform the requested action.".into(),
            ),
            ConverseStreamError::ResourceNotFoundException(e) => {
                e.message.unwrap_or("The specified resource ARN was not found.".into())
            }
            ConverseStreamError::ThrottlingException(e) => e.message.unwrap_or(
                "Your request was denied due to exceeding the account quotas for AWS Bedrock.".into(),
            ),
            ConverseStreamError::ServiceUnavailableException(e) => {
                e.message.unwrap_or("The service isn't currently available.".into())
            }
            ConverseStreamError::InternalServerException(e) => {
                e.message.unwrap_or("An internal server error occurred.".into())
            }
            ConverseStreamError::ModelStreamErrorException(e) => e.message.unwrap_or(
                "The request failed while the model was streaming a response.".into(),
            ),
            ConverseStreamError::ValidationException(e) => e.message.unwrap_or(
                "The input fails to satisfy the constraints specified by AWS Bedrock.".into(),
            ),
            ConverseStreamError::ModelNotReadyException(e) => e.message.unwrap_or(
                "The model specified in the request is not ready to serve inference requests.".into(),
            ),
            ConverseStreamError::ModelErrorException(e) => e.message.unwrap_or(
                "The request failed due to an error while processing the model.".into(),
            ),
            _ => "An unexpected error occurred. Verify Internet connection or AWS keys".into(),
        };
        CompletionError::transport(error)
